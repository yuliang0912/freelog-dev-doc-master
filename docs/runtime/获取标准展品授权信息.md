...
| 返回值字段 | 字段类型 | 字段说明 |
| :--- | :--- | :--- |
| freelog-article-nid | string | 当前响应的展品的依赖树链路ID |
| freelog-article-id | string | 当前请求的作品ID |
| freelog-article-name | string | 当前请求的作品名称 |
| freelog-article-sub-dependencies | string | 当前作品的依赖(包含依赖的作品ID,作品名称,作品类型和作品资源类型) |
| freelog-resource-type | string | 当前请求的作品的资源类型 |
| freelog-article-property | string | 昨天的属性信息,encodeURIComponent编码过的json字符串 |
