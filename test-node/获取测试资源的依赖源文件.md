...
```
https://api.freelog.com/v1/auths/testResources/{testResourceId}/subDepend{.extName}?subEntityId={subEntityId}&subEntityName={subEntityName}&subEntityType={subEntityType}&subEntityVersion={subEntityVersion}
...
| subEntityType | 必选 | string | 依赖实体的类型 |
| subEntityVersion | 可选 | string | 依赖实体的版本,类型是mock则可不传,是release需要对应的版本  |
...
| 返回值字段 | 字段类型 | 字段说明 |
| :--- | :--- | :--- |
| freelog-sub-entities | string | 当前测试资源子依赖,base64编码的json字符串 |
