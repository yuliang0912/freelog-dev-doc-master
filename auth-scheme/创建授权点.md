...
```
https://api.freelog.com/v1/resource/authSchemes
```
...
|policyText|必选|string| base64编码的授权策略 |
|languageType|可选|string|策略语言类型 默认为freelog_policy_lang|
|dependStatements|可选|object[]|授权点包含的以来声明|
|**resourceId|必选|string|资源依赖树上的引用节点资源ID|
|**deep|必选|int|节点所在的依赖树上的层级|
|**dependencies|必选|object[]|依赖树的层级子节点|
|****resourceId|必选|string|资源依赖树上的引用节点资源ID|
|****contractId|必选|string|声明依赖之间所绑定的合约|
